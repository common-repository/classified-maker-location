	...
	if(empty($_POST['classified_maker_loc_hidden']))
		{
			...
			$classified_maker_loc_options = get_option( 'classified_maker_loc_options' );
		...
		{	
			if($_POST['classified_maker_loc_hidden'] == 'Y') {
				...
				//$classified_maker_loc_options = stripslashes_deep($_POST['classified_maker_loc_options']);
				//update_option('classified_maker_loc_options', $classified_maker_loc_options);
	
	
			$class_classified_maker_loc_functions = new class_classified_maker_loc_functions();
			$settings_form_options = $class_classified_maker_loc_functions->setings_options();
...
<div class="wrap classified-maker-admin settings">
	...
	<h2><?php _e(sprintf('%s - Settings',classified_maker_loc_plugin_name), classified_maker_loc_textdomain); ?></h2>
		...
		<form  method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
			...
			$class_classified_maker_loc_form = new class_classified_maker_loc_form();
			
			
			
			
			$class_classified_maker_loc_functions = new class_classified_maker_loc_functions();
			$settings_form_input = $class_classified_maker_loc_functions->setings_options();
			...
			foreach($settings_form_input as $key=>$option_set){
				...
				foreach($option_set['options'] as $key=>$option ){
					...
					//var_dump($option);
					echo $class_classified_maker_loc_form->settings_form_input($option);
				...
				//echo $class_classified_maker_loc_form->settings_form_input($meta_fields[$key]);
				$i++;
				}
